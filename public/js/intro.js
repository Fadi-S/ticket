...
var lastStep = parseInt(getCookie('step'));
lastStep = isNaN(lastStep) ? 1 : lastStep;
window.addEventListener('load', function () {
  introJs().setOptions({
    tooltipClass: 'intro'
  }).goToStepNumber(lastStep).onchange(function (target) {
    var step = parseInt(target === null || target === void 0 ? void 0 : target.dataset.step);
    if (isNaN(step)) return;
    var lastStep = getCookie('step');

    if (step > lastStep) {
      lastStep = step;
      setCookie('step', lastStep, 365 * 10);
    }
  }).start();
});
})();
