<x-master>
    ...
    <x-slot name="title">View Kiahk {{ $kiahk->formatted_date }} | Ticket</x-slot>

    <x-card class="space-y-4">
        <div class="space-y-2">
            <h6 class="text-2xl font-semibold text-gray-800">
                {{ $kiahk->type->arabic_name }} | {{ $kiahk->formatted_date }} | {{ $kiahk->formatted_time }}
            </h6>

            <p class="text-sm text-gray-600">
                Places available: {{ $kiahk->number_of_places - $kiahk->reservedPlaces() . ' / ' . $kiahk->number_of_places}}
            </p>
        </div>
        ...
        <x-table.table>
            ...
            <x-slot name="body">
                @foreach($kiahk->reservations as $reservation)
                <tr>
                    <x-table.td>
