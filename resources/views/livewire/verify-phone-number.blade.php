<div>
    ...
    <div class="bg-white dark:bg-gray-700 duration-500 max-w-3xl mt-4
     mx-auto p-6 rounded-lg shadow-lg transition-colors">

    <h1 class="font-bold text-lg">{{ __('Verify Phone Number') }}</h1>

    <div class="flex flex-col space-y-4 items-start">
        <span class="text-sm text-gray-200">{{ __('To make sure that this is your phone number, we will send a verification code') }}</span>
        ...
        @if($edit)
            <livewire:users.user-form :user="auth()->user()" :only="['user.phone']" />
        @else
