...
<th
    {{ $attributes->merge(['class' => 'px-3 py-2 md:px-6 md:py-3 bg-gray-50 dark:bg-gray-900 transition-dark'])->only('class') }}
>
    @unless ($sortable)
        <span class="block text-left text-xs leading-4 font-medium text-gray-500 dark:text-gray-200 uppercase tracking-wider">
            {{ $text ?? $slot }}
        </span>
    @else
        <button
            ...
            class="flex items-center space-x-1 text-left text-xs leading-4 font-medium dark:text-gray-200 text-gray-500 uppercase tracking-wider group focus:outline-none focus:underline"
        >
            <span>{{ $text ?? $slot }}</span>
